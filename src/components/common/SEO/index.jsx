...
export const SEO = ({ title, type, description, location = '', readTime }) => {
  ...
  return (
    <Helmet>
      ...
      <meta
        property="og:title"
        content={title ? `MgrGracz | ${title}` : defaultTitle}
      />
      <meta
      ...
      <title>{title ? `MgrGracz | ${title}` : defaultTitle}</title>
      {type === 'NewsArticle' && (
      ...
      {type === 'NewsArticle' && (
        <meta name="author" content="Kamil Bandzwołek" data-react-helmet="true" />
      )}
